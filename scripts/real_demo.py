...
# 设置 DB
demo_db = r'D:\aiCode\Hermes\aiTest\ai-agent-governance\tokenkeeper\examples\real_demo.db'
...
def phase2_show_ledger():
    ...
    print('\n按模型汇总:')
    for row in summary:
        print(f'  {row["group_key"]:25s}  calls={row["calls"]:>3d}  cost=${row["cost_usd"]:.6f}')
...
def phase4_clear_and_redemo():
    ...
    for prompt in prompts:
        try:
            ...
            print(f'  ✅ {prompt[:30]!r:30s}  in={input_t:>4d}  out={output_t:>4d}  ${cost.cost_usd:.6f}')
        except Exception as e:
            print(f'  ❌ {e}')
...
def main():
    ...
    for row in summary:
        print(f'  {row["group_key"]:20s}  calls={row["calls"]:>3d}  cost=${row["cost_usd"]:.6f}')
